...
/* ============================================================
   ...
   難易度ティア:
     ...
     5〜6     = 上級   (谷ゲート3〜4・制限を引き締め)
     7〜10    = 超上級 (谷ゲート5〜7・深く間隔狭く、ノーミス&★3=最適走行を要求)
   ============================================================ */
...
function course7(){ const b=new CB();
  ...
  const gx=b.goal();
  return finishCourse(b,gx,{name:'ループ・コースター',en:'LOOP COASTER',limit:40,pal:5,seed:91,par:[19,26],
...
function course8(){ const b=new CB();
  ...
  b.flat(180).item('T',-80);
  const gx=b.goal();
  return finishCourse(b,gx,{name:'スカイブリッジ',en:'SKY BRIDGE',limit:40,pal:6,seed:101,par:[14,19],
...
function course9(){ const b=new CB();
  ...
  b.flat(150).item('N',-75).item('N',-150);
  b.sign('danger',20).hintN(0); b.rampUp(150,150); b.pit(205,310);
  b.flat(210).item('N',-110).item('T',-100);
  b.sign('danger',20).hintN(0); b.rampUp(150,152); b.pit(207,315);
  b.flat(210).item('N',-110);
...
function course10(){ const b=new CB();
  ...
  b.flat(200).item('T',-110);
  b.sign('up',30).hintN(10);
  b.plat(250,400,160).platItems(['N','T'],2);
  b.rampUp(150,110).cliffDown(110);
  b.flat(150);
  ...
  b.flat(180).item('T',-80);
  const gx=b.goal();
  return finishCourse(b,gx,{name:'ニトロ・ヘル',en:'NITRO HELL',limit:46,pal:7,seed:139,par:[20,26],
